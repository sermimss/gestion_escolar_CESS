...
def get_student(db: Session, student_id: str):
    ...
    return (
        ...
        .options(
            ...
            joinedload(models.Student.payment_history).joinedload(
                models.Transaction.covered_payments
            ),
            joinedload(models.Student.academic_history).joinedload(
                models.AcademicRecord.grades
            ),
...
def create_student(db: Session, student: schemas.StudentCreate):
    ...
    student_core_data = student.model_dump(
        exclude={
            "payments",
            "academic_history",
            "group",
    ...
    if student.secondary_certificate:
        db_student.secondary_certificate = models.Certificate(
            **student.secondary_certificate.model_dump()
        )
    if student.high_school_certificate:
        db_student.high_school_certificate = models.Certificate(
            **student.high_school_certificate.model_dump()
        )
    if student.work_experience:
        db_student.work_experience = models.WorkExperience(
            **student.work_experience.model_dump()
        )
    if student.contact:
    ...
    for payment_data in payments_data:
        ...
        db_payment = models.Payment(
            id=str(uuid.uuid4()), student_id=db_student.id, **payment_dict
        )
        db.add(db_payment)
    ...
    for record_data in academic_history_data:
        ...
        for grade_data in grades_data:
            db_grade = models.SubjectGrade(
                academic_record_id=db_record.id, **grade_data.model_dump()
            )
            db.add(db_grade)
