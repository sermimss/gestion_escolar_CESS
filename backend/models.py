...
transaction_payments_association = Table(
    ...
    Column(
        "payment_id", UUID(as_uuid=True), ForeignKey("payments.id"), primary_key=True
    ),
)
...
class Student(Base):
    ...
    secondary_certificate = relationship(
        "Certificate",
        foreign_keys="[Certificate.student_id_secondary]",
        back_populates="student_secondary",
        uselist=False,
        cascade="all, delete-orphan",
    )
    high_school_certificate = relationship(
        "Certificate",
        foreign_keys="[Certificate.student_id_highschool]",
        back_populates="student_highschool",
        uselist=False,
        cascade="all, delete-orphan",
    )
    ...
    contact = relationship(
        "Contact", back_populates="student", uselist=False, cascade="all, delete-orphan"
    )
    ...
    payment_history = relationship(
        "Transaction", back_populates="student", cascade="all, delete-orphan"
    )
    academic_history = relationship(
        "AcademicRecord", back_populates="student", cascade="all, delete-orphan"
    )
    documentation_files = relationship(
        "DocumentationFile", back_populates="student", cascade="all, delete-orphan"
    )
...
class Certificate(Base):
    ...
    student_id_secondary = Column(
        String, ForeignKey("students.id"), unique=True, nullable=True
    )
    student_id_highschool = Column(
        String, ForeignKey("students.id"), unique=True, nullable=True
    )

    student_secondary = relationship(
        "Student",
        foreign_keys=[student_id_secondary],
        back_populates="secondary_certificate",
    )
    student_highschool = relationship(
        "Student",
        foreign_keys=[student_id_highschool],
        back_populates="high_school_certificate",
    )
...
class Transaction(Base):
    ...
    covered_payments = relationship(
        "Payment", secondary=transaction_payments_association
    )
...
class AcademicRecord(Base):
    ...
    grades = relationship(
        "SubjectGrade", back_populates="academic_record", cascade="all, delete-orphan"
    )
